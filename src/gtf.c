...
#define MAGIC_BYTES {0x47, 0x54, 0x41, 0x42}

#define END_BYTES {0xFF, 0xF1, 0xFF, 0xF1}
...
int compare_event(Event *e1, Event *e2) {
  uint32_t e1_delta_time;
  // first entry in data is always deltatime;
  memcpy(&e1_delta_time, e1->data, sizeof(int32_t));

  uint32_t e2_delta_time;
  // first entry in data is always deltatime;
  memcpy(&e2_delta_time, e2->data, sizeof(int32_t));
...
int32_t get_quater_note(NoteOnEvent *event, TimeSignature timeSig,
                        int32_t tempo) {
  int64_t numer = timeSig.numerator * MICRO_SECOND;
  int64_t denom = timeSig.denominator * MICRO_SECOND;
  // u_int32_t fraction_of_measure = 1.f / denom;

  int64_t one_beat = (60 * MICRO_SECOND) / (tempo);
  printf("one beat: %llu\n", one_beat);

  int64_t note_length = (one_beat * numer) / denom;

  printf("q note: %llu\n", note_length);
  return note_length;
}

NoteLength note_type(NoteOnEvent *event, TimeSignature timeSig, int32_t tempo) {
...
int write_gtab(GTabHeader *header, const char *file_name) {
  ...
  fwrite(&events.count, sizeof(int32_t), 1, file);
